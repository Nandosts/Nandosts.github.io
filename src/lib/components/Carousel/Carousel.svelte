<script lang="ts">
  ...
  let openedImage: number | null = null;
  let innerWidth = 0;
  ...
  const redirectToSite = async (id: number | string) => {
    await goto(`/Sites/${id as string}`);
  };
...
<div id="carousel-container">
  <div id="carousel-sites" style={`padding-inline: ${horizontalPadding}`}>
    {#each sites as site, i (site.id)}
      <img
        src={openedImage === i || innerWidth < 500 ? site.openedNotebook : site.carouselImage}
        alt={site.name}
        id={site.id}
...
<style lang="scss">
  ...
  #carousel-sites {
    ...
    flex-wrap: nowrap;
    -webkit-mask: linear-gradient(to right, transparent, black 30%, black 70%, transparent);
